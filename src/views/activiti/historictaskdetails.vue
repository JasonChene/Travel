<template>
  <div class="app-container">
    ...
    <el-row class="el-row-item">
      <el-col :span="12">{{ $t('task.name') }}：{{ this.$route.query && this.$route.query.name }}</el-col>
      <el-col :span="12">{{ $t('task.status') }}：{{ this.$route.query && this.$route.query.status }}</el-col>
    </el-row>
    <el-row class="el-row-item">
      <el-col :span="12">{{ $t('process.activity-starttime') }}：{{ this.$route.query && this.$route.query.startTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</el-col>
      <el-col :span="12">{{ $t('process.activity-endtime') }}：{{ this.$route.query && this.$route.query.endTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</el-col>
    </el-row>
    <el-divider />
    <div style="margin:10px 0 5px 0px">
      {{ $t('process.comments') }}
