<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('process.executionName')" prop="name" />
      ...
      <el-table-column :label="$t('process.ended')">
        <template slot-scope="{row}">
          <span v-if="row.ended === false">{{ $t('common.no') }}</span>
          <span v-else>{{ $t('common.yes') }}</span>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { queryExecution } from '@/api/activiti'
